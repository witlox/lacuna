...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('parent_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('classifications.id')),
        sa.Column('metadata', postgresql.JSON(), default=dict),
    )

    op.create_index(
        'idx_classification_tier_timestamp',
        'classifications',
        ['tier', 'timestamp']
    )
    ...
    op.create_table(
        ...
        sa.Column('source_classification_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('classifications.id')),
        sa.Column('target_classification_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('classifications.id')),
        sa.Column('metadata', postgresql.JSON(), default=dict),
    )
    ...
    op.create_index(
        'idx_lineage_source',
        'lineage_edges',
        ['source_artifact_id', 'timestamp']
    )

    op.create_index(
        'idx_lineage_target',
        'lineage_edges',
        ['target_artifact_id', 'timestamp']
    )
    ...
    op.create_index('idx_audit_resource_timestamp', 'audit_log', ['resource_id', 'timestamp'])
    op.create_index('idx_audit_classification_timestamp', 'audit_log',
                    ['resource_classification', 'timestamp'])
    op.create_index('idx_audit_event_type', 'audit_log', ['event_type', 'timestamp'])
    op.create_index('idx_audit_action_result', 'audit_log', ['action_result', 'timestamp'])
    ...
    op.create_index('idx_policy_eval_user', 'policy_evaluations', ['user_id', 'timestamp'])
    op.create_index('idx_policy_eval_resource', 'policy_evaluations', ['resource_id', 'timestamp'])
    op.create_index('idx_policy_eval_result', 'policy_evaluations', ['allowed', 'timestamp'])
